...
(function() {
	...
	var version_modss = '3.0',
		...
		leftVipD = ' 💎 26 дней',
		user_id = 533867052,
		uid = '72ba41c6eee8f7862b7c57cbc956fe2f_533867052',
		IP, logged = true;
	console.log('App', 'protocol:', Protocol());
	console.log('Modss', 'api:', API);
	...
	var Modss = {
		init: function() {
			...
			if (!logged && vip) setTimeout(function() {
				Modss.auth();
			}, 100);
		...
		rating_kp_imdb: function(card) {
			return new Promise(function(resolve, reject) {
				...
				Pub.network.silent(API + 'KPrating', function(json) {
					...
					if (!ping_auth && auth == 'pending') Modss.auth(true);
					vip = json.data.vip;
		...
		balansers: function() {
			var balansers = {
				"hdr": "HDR MODS's [4K]  <span style=\"font-weight: 700;color:rgb(236,151,31)\">VIP</span>",
			...
			try {
				var items = component.parsePlaylist(str).map(function(item) {
					...
					links = item.links.filter(function(url) {
						return /\.m3u8$/i.test(url); //return /\.mp4$/i.test(url);
					});
	...
	function component(object) {
		...
		this.requestParams = function(url) {
			...
			query.push('uid=72ba41c6eee8f7862b7c57cbc956fe2f_533867052');
			if (cards) {
				var notices = Lampa.Storage.get('account_notice', []).filter(function(n) {
		...
		this.checkHashR = function(params) {
			return new Promise(function(resolve, reject) {
				...
				xhr.ontimeout = function() {
					console.log('Modss', 'reserve_url');
					params.url = params.url_reserve;
					params.quality = params.quality_reserve;
					resolve(params);
				};
		...
		this.getFileUrl = function(file, call) {
			...
			if (balanser == 'qiwi') {
				...
				if (['zetflix', 'videocdn', 'videodb'].indexOf(balanser) >= 0 && 'RU' == 'UA') {
					console.log('chURL')
				...
				network["native"](file.url, function(json) {
					if (['hdrezka', 'kodik'].indexOf(balanser) >= 0 && 'RU' == 'UA') {
		...
		*/
		...
		function add() {
			...
			Lampa.SettingsApi.addParam({
				...
				field: {
					name: '🆔 533867052 🔹  📱 🤖 Android 13 (RMX3242)'
				}
			});
			Lampa.SettingsApi.addParam({
				component: 'settings_modss',
